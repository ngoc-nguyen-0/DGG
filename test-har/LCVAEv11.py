...
def layerwise_train(model,dataloader,optimizer,lr_scheduler,epoch_num=100,use_gpu=torch.cuda.is_available()):
	if use_gpu:
		model = model.cuda()
	...
	for epochs in range(epoch_num):
		...
		for batch_idx, batch in enumerate(dataloader):
			...
			vis_recon = model(getcorrupt(inputs,sigma))
			# vis_recon = model(inputs)
			loss = torch.sum(torch.pow(inputs-vis_recon/(1-sigma),2))/inputs.size()[0]
			optimizer.zero_grad()
			loss.backward()
			optimizer.step()
...
class GMM_Model(nn.Module):
	...
	def compute_prob(self,data):
		...
		pc = torch.div(prob,(torch.sum(prob,dim=-1)).view(-1,1))		
		return pc
...
# def train(vae,classifer,GMM,optimizer1,optimizer2,lr_scheduler1,lr_scheduler2,dataloader,epoch_num,use_gpu=torch.cuda.is_available()):
	...
	for epoch in range(epoch_num):
		...
		for batch_idx, batch in enumerate(dataloader):
			...
			for idx in range(J):
				...
				ELBO = ELBO + 100*torch.sum(torch.mul(log_recon,weight))
...
if __name__ == '__main__':
	...
	for idx in range(1):
		data_temp = torch.from_numpy(torch.load('data_NN/har/har_{}_all_siamese.pkl'.format(idx))).unsqueeze(2)
		image_train.append(data_temp)
	'''
	for idx in range(21):
		data_temp = torch.from_numpy(torch.load('data_NN/har/har_{}_all_real.pkl'.format(idx))).unsqueeze(2)
		image_train.append(data_temp)
	'''
	image_train = to_numpy(torch.cat(image_train,dim=-1))
	image_train = image_train/2

	label_train = torch.load('data_NN/har/har_label_all_siamese.pkl')
	...
	print(sum((label_train==4)*1.0))
	print(sum((label_train==5)*1.0))


	print('| Training data')
	print("| Data shape: {}".format(image_train.shape))
	print("| Data range: {}/{}".format(image_train.min(), image_train.max()))
	print("| Label range: {}/{}".format(label_train.min(), label_train.max()))
	...
	if not resume:
		if use_trainedweight:
			...
			w_init.append(torch.tensor(np.loadtxt('./pretrained_weight/har/har_w_7.out',delimiter=',').astype(np.float32)))
			w_init.append(torch.tensor(np.loadtxt('./pretrained_weight/har/har_b_7.out',delimiter=',').astype(np.float32)))
			...
			w_init.append(torch.tensor(np.loadtxt('./pretrained_weight/har/har_w_6.out',delimiter=',').astype(np.float32)))
			w_init.append(torch.tensor(np.loadtxt('./pretrained_weight/har/har_b_6.out',delimiter=',').astype(np.float32)))
			...
			w_init.append(torch.tensor(np.loadtxt('./pretrained_weight/har/har_w_5.out',delimiter=',').astype(np.float32)))
			w_init.append(torch.tensor(np.loadtxt('./pretrained_weight/har/har_b_5.out',delimiter=',').astype(np.float32)))
			...
			w_init.append(torch.tensor(np.loadtxt('./pretrained_weight/har/har_w_4.out',delimiter=',').astype(np.float32)))
			w_init.append(torch.tensor(np.loadtxt('./pretrained_weight/har/har_b_4.out',delimiter=',').astype(np.float32)))
	...
	vae = vae.cuda()
	gmm = mixture.GaussianMixture(n_components=6, covariance_type='spherical',n_init=10,init_params='kmeans')
	...
	var = np.tile(var.astype(np.float32),[10,1])
	# var = var.astype(np.float32)
	# print(var)
	'''
	x_mean = to_numpy(x_mean)
	kmeans = KMeans(n_clusters=n_cls,random_state=0).fit(x_mean)
	cls_idx = kmeans.labels_
	centers = kmeans.cluster_centers_
	var = []
	for idx in range(L):
		index = np.where(cls_idx)
	...
	kmeans = KMeans(n_clusters=6,random_state=0).fit(to_numpy(x_mean))
	...
	# GMM = GMM_Model(10,L,mean,var.transpose())
	GMM.eval()
	GMM = GMM.cuda()
	label_pred = torch.max(GMM.compute_prob(x_mean),dim=-1)
	# print(label_pred[-1].max())
	acc,_ = cluster_acc(label_train,to_numpy(label_pred[-1]))
	NMI = metrics.normalized_mutual_info_score(label_train, to_numpy(label_pred[-1]),average_method='arithmetic')
	# label_pred = torch.from_numpy(gmm.predict(to_numpy(x_mean)))
	# label_pred = torch.max(aa,dim=-1)
	...
	dataloader = DataLoader(TensorDataset(torch.from_numpy(image_train[:,:,0]),torch.from_numpy(label_train)),batch_size=BATCH_SIZE,shuffle=True)
	# optimizer = optim.Adam(classifer.parameters(),lr=0.0001,weight_decay=0.0001)
	optimizer = optim.SGD(classifer.parameters(),lr=0.0001,momentum=0.9)
	# optimizer = optim.Adam(classifer.parameters(),lr=0.0001)
	lr_scheduler = StepLR(optimizer,step_size=170,gamma=0.1)
	print("2.1 pretrain the classifer")
	classifer= pretrain_classifer(vae,GMM,classifer,optimizer,lr_scheduler,dataloader,epoch_num=50)
	...
	# optimizer = optim.SGD(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.0001,momentum=0.9,weight_decay=0.001)
	optimizer = optim.Adam(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.0002)
	# optimizer = optim.RMSprop(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.0002,weight_decay=0.0001)
	...
	'''
	optimizer1 = optim.Adam(list(vae.get_para())+list(classifer.parameters()),lr=0.002,weight_decay=0.0001)
	optimizer2 = optim.Adam(list(GMM.get_para()),lr=0.002)
	# optimizer = optim.Adam(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.002,weight_decay=0.0001)
	# optimizer = optim.SGD(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.01,momentum=0.9,weight_decay=0.001)
	# optimizer = optim.Adam(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.0002)
	# optimizer = optim.RMSprop(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.0002,weight_decay=0.0001)
	lr_scheduler1 = StepLR(optimizer1,step_size=10,gamma=0.9)
	lr_scheduler2 = StepLR(optimizer2,step_size=10,gamma=0.9)
	print("2.2 Train the model")
	vae,classifer,GMM = train(vae,classifer,GMM,optimizer1,optimizer2,lr_scheduler1,lr_scheduler2,dataloader,epoch_num=300)
	vae.eval()
	classifer.eval()
	x_mean,_ = vae.get_latent(torch.from_numpy(image_train[:,:,0]).cuda())
	pred_l = torch.max(classifer(x_mean),dim=-1)

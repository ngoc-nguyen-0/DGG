...
def Form_data(data):
	K = 5
	similar_m = to_numpy(cal_similar(data,K,'cosine'))
...
def train(siamese,optimizer,lr_scheduler,dataloader,epoch_num,use_gpu=torch.cuda.is_available()):
	...
	for epoch in range(epoch_num):
		...
		for batch_idx, batch in enumerate(dataloader):
			...
			distance = torch.sqrt(torch.sum(torch.pow(latent1-latent2,2),dim=1))
			# print(distance.type())
			# print(label.type())
			# print(c.type())
			# assert 1 == 0
			# loss = torch.sum(torch.max(c+torch.mul(label,distance),th)**2)/inputs1.size(0)
			loss = (torch.sum(torch.mul(label,distance)**2)+torch.sum(torch.max(c-torch.mul(1-label,distance),th)**2))/inputs1.size(0)
...
if __name__ == '__main__':
	...
	if not resume:
		...
		optimizer = optim.SGD(siamese.parameters(),lr=0.02,momentum=0.9)
	...
	kmeans = KMeans(n_clusters=10,random_state=0).fit(image_train)
	cls_index = kmeans.labels_
	mean = kmeans.cluster_centers_
	acc,_ = cluster_acc(cls_index,label_train)
	NMI = metrics.normalized_mutual_info_score(label_train, cls_index,average_method='arithmetic')
	...
	similar_m_latent = cal_similar(latent,100,'cosine')
	# similar_m_latent = cal_similar(latent,20)
	similar_m_latent = random_select(similar_m_latent,20)
	...
	similar_m_ori = cal_similar(torch.from_numpy(image_train),20,'cosine')
	# similar_m_ori = random_select(similar_m_ori,20)
	# torch.save(similar_m_ori,'similar_m.pkl')
	# similar_m_ori = cal_similar(torch.from_numpy(image_train),20)
	# similar_m_ori = cal_similar(torch.from_numpy(image_train),5)
	print("")
	print("| Evaluate the NN")

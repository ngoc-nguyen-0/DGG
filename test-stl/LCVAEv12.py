...
def layerwise_train(model,dataloader,optimizer,lr_scheduler,epoch_num=1000,use_gpu=torch.cuda.is_available()):
	if use_gpu:
		model = model.cuda()
	...
	def __init__(self,layer_sizes,inputs,activation=nn.ReLU(),w_init=None):
		...
		for idx in range(n_layer-1):
			...
			if w_init is not None:
				...
				layer_model = layerwise_train(layer_model,dataloader,optimizer,lr_scheduler,epoch_num=100)
		...
		self.var_layer = AE_layer1("Last",layer_sizes[-2],layer_sizes[-1])
...
def pretrain(vae, optimizer,lr_scheduler,dataloader,epoch_num,use_gpu=torch.cuda.is_available()):
	...
	for epoch in range(epoch_num):
		...
		for batch_idx, batch in enumerate(dataloader):
			...
			ELBO_rec = - 0.5*torch.sum(torch.pow(inputs-x_re,2))
			ELBO_reg = 0
			...
			loss = (-0.1*ELBO_rec - ELBO_reg)/inputs.size(0)
			optimizer.zero_grad()
			loss.backward()
			optimizer.step()
			Total_loss.append(loss.item())
...
def to_onehot(data,D):
	label = torch.zeros(data.size(0),D)
	label[:,data] = 1
	return label
...
def pretrain_classifer(vae,GMM,classifer,optimizer,lr_scheduler,dataloader,epoch_num,use_gpu=torch.cuda.is_available()):
	...
	for epoch in range(epoch_num):
		...
		for batch_idx, batch in enumerate(dataloader):
			...
			# temp = torch.max(prob_target,dim=-1)
			# prob_target = to_onehot(temp[-1],10).cuda()

			cond_prob = classifer(x_mean)

			loss = -torch.sum(torch.mul(prob_target,torch.log(cond_prob)))#-torch.sum(torch.mul(1-prob_target,torch.log(1-cond_prob)))
			optimizer.zero_grad()
			loss.backward()
			optimizer.step()
			Total_loss.append(loss.item())
...
def train(vae,classifer,GMM,optimizer,lr_scheduler,dataloader,epoch_num,use_gpu=torch.cuda.is_available()):
	...
	for epoch in range(epoch_num):
		...
		for batch_idx, batch in enumerate(dataloader):
			...
			x_mean, x_logvar = vae.get_latent(inputs)
			# print(torch.mean(torch.exp(x_logvar)))
			J = 1
			...
			x_mean_temp = []
			x_mean_temp.append(torch.zeros(N_samples,x_mean.size(1)).cuda())
			for idx in range(N_n-1):
				x_mean_temp.append(x_mean[:N_samples,:])

			x_mean_temp = torch.cat(x_mean_temp,dim=0)

			loss_a = torch.sum(torch.mul(torch.sum(torch.pow(x_mean,2)-2*torch.mul(x_mean,x_mean_temp),dim=1),weight))/N_samples
			...
			# loss = loss - ELBO/N_samples + loss_a
			...
			re_loss = F.mse_loss(x_re,inputs[:N_samples,:])*N_samples
...
if __name__ == '__main__':
	...
	if dataset == "mnist":
		...
		encoder_sizes = [2048, 500, 500, 2000, 10]
		# encoder_sizes = [2048, 1200, 1200, 10]
		classifer_sizes = [10,L,L]
	...
	label_train = torch.load('data_NN/stl10/stl10_label_all_siamese.pkl')
	...
	if resume:
		...
		vae = torch.load('pretrained_vae_n.pkl')
	...
	dataloader = DataLoader(TensorDataset(torch.from_numpy(image_train[:,:,0]),
									      ...
									      torch.from_numpy(cls_index.astype(np.long))),batch_size=1000,shuffle=True)
	optimizer = optim.Adam(classifer.parameters(),lr=0.01,weight_decay=0.0001)
	# optimizer = optim.SGD(classifer.parameters(),lr=0.2,momentum=0.9)
	# optimizer = optim.Adam(classifer.parameters(),lr=0.01)
	lr_scheduler = StepLR(optimizer,step_size=700,gamma=0.1)
	...
	# optimizer = optim.SGD(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.001,momentum=0.9,weight_decay=0.001)
	# optimizer = optim.SGD(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.001,momentum=0.9)
	optimizer = optim.Adam(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.0005,weight_decay=0.0001)
	# optimizer = optim.RMSprop(list(vae.get_para())+list(classifer.parameters())+list(GMM.get_para()),lr=0.0002,weight_decay=0.0001)
	lr_scheduler = StepLR(optimizer,step_size=70,gamma=0.9)
